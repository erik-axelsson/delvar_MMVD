...
################################################################################
### This script annotates nonredundent structural variants called using      ###
### the CNVDiscovery pipeline within GenomeStrip.                            ###
### Annotations include: Breedwise VST, gene, transcript, effect, phastcons, ###
### GO-terms and allele frequencies.                                         ###
################################################################################
 

$SV_TYPE="CNV"; #"DEL" for SVGenotyper pipeline, "CNV" for CNVDiscovery pipe line.
$min_fst=0.0; #set fst/vst threshold for reporting structural variants 
$promoter=1000; #define promoter region as region within X distance from transcription start 

system "touch /proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/tuk.txt";
system "touch /proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/cnv_VST.txt";
system "touch /proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/cnv_sd.txt";
system "touch /proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/anova_sd.txt";
system "touch /proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/anova.txt";
...
foreach $chr (sort @chromosomes) {
    $infile = "/proj/uppstore2017236/b2013119_nobackup/private/160DG_GenomeSTRIP/CNVDiscovery/".$chr."/results/gs_cnv.genotypes.annotated.vcf";
    print "$infile\n";
    push @infiles, $infile;
}


open OUT, ">>/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/".$SV_TYPE."/ANNOTATED_".$SV_TYPE.".190425.txt";

print OUT "chr\tstart\tend\tlength\tid\tNA\tNA\tgene_name\tgene\ttranscript\teffect\tdescription\tphastcon\tphastcon_lod\tbeagle_VST\tckcs_VST\tgs_VST\tgr_VST\tlr_VST\tpoodle_VST\trotw_VST\twhwt_VST\ttotal_VST\tANOVA\tGO-annotation\tbeagle_CNV\tckcs_CNV\tgs_CNV\tgr_CNV\tlr_CNV\tpoodle_CNV\trotw_CNV\twhwt_CNV\tbeagle_TUKEY_sign\tckcs_TUKEY_sign\tgs_TUKEY_sign\tgr_TUKEY_sign\tlr_TUKEY_sign\tpoodle_TUKEY_sign\trotw_TUKEY_sign\twhwt_TUKEY_sign\tbeagle_range\tCKCS_range\tgs_range\tgr_range\tlr_range\tpoodle_range\trotw_range\twhwt_range\n";       
...
#        print "@BREED_ORDER\n";
      ...
      if ($line!~/^#/) { 
	  %FREQ=%A=%B=%ALLELE_FREQS=%SITE_VISE_IND_COUNT=%IND_CNV=%IND_BREED=%TUK_COUNT=%COPY_NUMBERS=%IND_ALLELE_COUNT=();
	  $sample_tracker=$allele_count=$tot_freq_test=$effect=$high_fst_flag=0;
...
#Example of an individual call at CNV
#GT:CN:CNF:CNL:CNP:CNQ:FT        .:2:1.9498:-266.77,-12.52,-0.00,-5.24,-14.91:-269.97,-14.75,-0.00,-7.44,-54.34:74.4:PASS 

#Genotype  :  "Copy number genotype for imprecise events"  :  "Estimate of fractional copy number"  :  "Copy number likelihoods with no frequency prior"  :  "Copy number likelihoods"  :  "Copy number genotype quality for imprecise events"  :  "Per-sample genotype filter"

#	      print "$header[$i]\n";
	      if ($header[$i]=~/(.+?)\:(.+?)\:(.+?)\:(.+?)\:(.+?)\:(\d+?\..+?)\:*/) { #(.+)/) {
		  $GT=$1;
		  $CN=$2;  #copynumber
		  $FCN=$3; #fractional copy number
#		  $FI=$7;  #per sample genotype filter
...
#		  print "$GT\t$CN\t$FCN\t$GQ\n";
#		  die if ($GQ<13);
#		  print STDERR "		  next if ($GQ<13);\n";
		  
		  next if ($GQ<13); #skip genotypes with quality less then 13 (marked as LQ by GENOMEstrip
#		  die;
#		  $GL=$2;
#		  $GQ=$3;
#		  $FT=$4;
...
#		  $RR=$8;
#		  $RV=$9;
#		  print "$header[2]\t$i\t$BREED_ORDER[$i-9]\t$CN\n";
...
#		      unless ($genotype eq ".") {
		  ...
		  #Store copy numbers per breed and sample
		  $IND_CNV{$i}=$CN;
		  $IND_ALLELE_COUNT{$CN}++; #count how many times each allele is seen in population
		  $ind_breed=$BREED_ORDER[($i-9)];
		  $ind_breed=~s/\s//g;
		  $IND_BREED{$i}=$ind_breed;
		  $COPY_NUMBERS{$ind_breed}.= $CN.'_';
#		  print STDERR "$COPY_NUMBERS{$ind_breed}.= $CN.'_';\n";
		  $ALLELE_FREQS{$BREED_ORDER[$i-9]}+=$CN;
		  $CHROMOSOMES{$BREED_ORDER[$i-9]}++; # Number of individuals sampled per breed
		  unless ($CN == $cnv_test) {
		      $change++;
		      $cnv_test=$CN;
		  }
...
#		  }
		  ...
		  print "failed to match $header[$i]\n";
		  die;
	      }
	}
	  ...
	  $confirmed=0;
	  foreach $allele (sort keys %IND_ALLELE_COUNT) { #count how many times each allele is	
	      print "$chr\t$pos\t$allele\t$IND_ALLELE_COUNT{$allele}\n";
	      if ($IND_ALLELE_COUNT{$allele}>1) {
		  $confirmed++;
	      }
	  }
	  next LINE unless ($confirmed>1);
	  
	  
	  ### Send data to ANOVA in R ###
	  $anova_file="/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/cnv_anova.txt";
	  system "rm $anova_file";
	  system "rm /proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/tuk.txt";
	  system "rm /proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/anova.txt";
	  system "rm /proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/cnv_VST.txt";
	  open AOUT, ">>$anova_file" or die;
	  print AOUT "cnv\tbreed\n";
	  foreach $ind (sort numerically keys %IND_CNV) {
#	      print "$ind\t$IND_CNV{$ind}\n";
	      print AOUT "$IND_CNV{$ind}\t$IND_BREED{$ind}\n";
	  }
	  close AOUT;
	  
	  ### R calculations ###
	  $sd_results="/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/cnv_sd.txt";
	  system "rm $sd_results";
	  
	  system "R CMD BATCH /proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/anova.r";
	  
	  ### Load R results ###
	  $anova_results = "/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/anova.txt";
	  $tukey_results="/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/tuk.txt";
	  $sd_results="/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/cnv_sd.txt";
	  $vst_results="/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/cnv_VST.txt";
	  open ANOV, $anova_results or die;
	  open TUKEY, $tukey_results or die;
	  open SD, $sd_results or die;
	  open VST, $vst_results or die;
...
#	  foreach $breed (sort keys %VST) {
#	      print "hel\t$breed\t$VST{$breed}\n";
#	  }
#	  print "$tot_VST\n";
#	  die;
...
#	  }
	  ...
	  $results="chr\tstart\tend\tlength\tid\tNA\tNA\tgene_name\tgene\ttranscript\teffect\tdescription\tphastcon\tphastcon_lod\tbeagle_VST\tckcs_VST\tgs_VST\tgr_VST\tlr_VST\tpoodle_VST\trotw_VST\twhwt_VST\ttotal_VST\tANOVA\tGO-annotation\tbeagle_CNV\tckcs_CNV\tgs_CNV\tgr_CNV\tlr_CNV\tpoodle_CNV\trotw_CNV\twhwt_CNV\tbeagle_TUKEY_sign\tckcs_TUKEY_sign\tgs_TUKEY_sign\tgr_TUKEY_sign\tlr_TUKEY_sign\tpoodle_TUKEY_sign\trotw_TUKEY_sign\twhwt_TUKEY_sign\tbeagle_range\tCKCS_range\tgs_range\tgr_range\tlr_range\tpoodle_range\trotw_range\twhwt_range\n";

#	  $results="chr\tstart\tend\tlength\tid\tNA\tNA\tgene_name\tgene\ttranscript\teffect\tdescription\tphastcon\tphastcon_lod\tbeagle_VST\tckcs_VST\tgs_VST\tgr_VST\tlr_VST\tpoodle_VST\trotw_VST\twhwt_VST\ttotal_VST\tANOVA\tGO-annotation\tbeagle_CNV\tckcs_CNV\tgs_CNV\tgr_CNV\tlr_CNV\tpoodle_CNV\trotw_CNV\twhwt_CNV\tbeagle_TUKEY_sign\tckcs_TUKEY_sign\tgs_TUKEY_sign\tgr_TUKEY_sign\tlr_TUKEY_sign\tpoodle_TUKEY_sign\trotw_TUKEY_sign\twhwt_TUKEY_sign\t$beagle_range\t$CKCS_range\t$gs_range\t$gr_range\t$lr_range\t$poodle_range\t$rotw_range\t$whwt_range\n";
#	  $results="chr\tstart\tend\tlength\tid\tNA\tNA\tgene_name\tgene\ttranscript\teffect\tdescription\tphastcon\tphastcon_lod\tbeagle_cnv\tckcs_cnv\tgs_cnv\tgr_cnv\tlr_cnv\tpoodle_cnv\trotw_cnv\twhwt_cnv\tANOVA\tGO-annotation";
...
#	  $pre_text= "$header[0]\t$header[1]\t$end_SV\t$length_SV\t$header[2]\t";
	  ...
	  ### Calculate breed specific allele frequencies for SNP ###
	  $j=25;
	  foreach $breed (sort keys %CHROMOSOMES) {
		  if (defined ($ALLELE_FREQS{$breed}) && ($CHROMOSOMES{$breed})) {         
		      #average copy number per breed
		      $freq=$ALLELE_FREQS{$breed}/$CHROMOSOMES{$breed};
		      $rt[$j].="$freq";
		      $j++;
		  }
	  }
	  ...
	  $array_pos=14;
	  foreach $breed (sort keys %VST) {
#	      print STDERR "	      $rt[$array_pos].=$VST{$breed};\n";
	      $rt[$array_pos].=$VST{$breed};
	      $array_pos++;
	  }
	  $rt[22]=$tot_VST;
	  ...
	  if ($p_anova<0.05) {
	      if (exists $TUK_COUNT{'Beagle'}) {
		  $rt[33]=$TUK_COUNT{'Beagle'};
	      }else{
		  $rt[33]=0;
	      }
	      if (exists $TUK_COUNT{'CavalierKingCharlesSpaniel'}) {
		  $rt[34]=$TUK_COUNT{'CavalierKingCharlesSpaniel'};
	      }else{
		  $rt[34]=0;
	      }
	      if (exists $TUK_COUNT{'GermanShephard'}) {
		  $rt[35]=$TUK_COUNT{'GermanShephard'};
	      }else{
		  $rt[35]=0;
	      }
	      if (exists $TUK_COUNT{'GoldenRetriever'}) {
		  $rt[36]=$TUK_COUNT{'GoldenRetriever'};
	      }else{
		  $rt[36]=0;
	      }
	      if (exists $TUK_COUNT{'LabradorRetriever'}) {
		  $rt[37]=$TUK_COUNT{'LabradorRetriever'};
	      }else{
		  $rt[37]=0;
	      }
	      if (exists $TUK_COUNT{'Poodle'}) {
		  $rt[38]=$TUK_COUNT{'Poodle'};
	      }else{
		  $rt[38]=0;
	      }
	      if (exists $TUK_COUNT{'Rottweiler'}) {
		  $rt[39]=$TUK_COUNT{'Rottweiler'};
	      }else{
		  $rt[39]=0;
	      }
	      if (exists $TUK_COUNT{'WestHighlandWhiteTerrier'}) {
		  $rt[40]=$TUK_COUNT{'WestHighlandWhiteTerrier'};
	      }else{
		  $rt[40]=0;
	      }
	  }

	  if (exists $COPY_NUMBERS{'Beagle'}) {
	      $rt[41]=$COPY_NUMBERS{'Beagle'};
	  }else{
	      $rt[41]=0;
	  }
	  if (exists $COPY_NUMBERS{'CavalierKingCharlesSpaniel'}) {
	      $rt[42]=$COPY_NUMBERS{'CavalierKingCharlesSpaniel'};
	  }else{
	      $rt[42]=0;
	  }
	  if (exists $COPY_NUMBERS{'GermanShephard'}) {
	      $rt[43]=$COPY_NUMBERS{'GermanShephard'};
	  }else{
	      $rt[43]=0;
	  }
	  if (exists $COPY_NUMBERS{'GoldenRetriever'}) {
	      $rt[44]=$COPY_NUMBERS{'GoldenRetriever'};
	  }else{
	      $rt[44]=0;
	  }
	  if (exists $COPY_NUMBERS{'LabradorRetriever'}) {
	      $rt[45]=$COPY_NUMBERS{'LabradorRetriever'};
	  }else{
	      $rt[45]=0;
	  }
	  if (exists $COPY_NUMBERS{'Poodle'}) {
	      $rt[46]=$COPY_NUMBERS{'Poodle'};
	  }else{
	      $rt[46]=0;
	  }
	  if (exists $COPY_NUMBERS{'Rottweiler'}) {
	      $rt[47]=$COPY_NUMBERS{'Rottweiler'};
	  }else{
	      $rt[47]=0;
	  }
	  if (exists $COPY_NUMBERS{'WestHighlandWhiteTerrier'}) {
	      $rt[48]=$COPY_NUMBERS{'WestHighlandWhiteTerrier'};
	  }else{
	      $rt[48]=0;
	  }
	  
#	      $j=41;
#	      foreach $breed (%COPY_NUMBERS) {
#		  unless ($breed=~/ARRAY/) {
#		      $rt[$j]=$COPY_NUMBERS{$breed};
#		      print "hej\t$breed\n";
#		      print "$breed\t$COPY_NUMBERS{$breed}\n";
#		      $j++;
#		  }
#	      }
#	      foreach $breed (sort keys %TUK_COUNT) {
#		  print "$breed\t$TUK_COUNT{$breed}\n";
#	      }
#	      if ($tuk_flag==1) {
#		  die;
#	      }
...
#	  push (@{$RESULTS{$chr}{$sv_start}}, "$chr,$sv_start,$sv_stop");
	  print "inside gene\t";
		  ...
		  $rt[9]="$TRANSCRIPT_ID{$chr}{$transcr_start}";
#		  $rt[10]="transcribed";
		  print "$gene_id\t$GENE_NAME{$gene_id}\n";
#		  $pre_text.= "\t\t\t$GENE_ID{$chr}{$transcr_start}\t$TRANSCRIPT_ID{$chr}{$transcr_start}\t";
...
#	  print "$i\t$rt[$i]\n";
      ...
      print OUT "\n";
#      die if ($rt[4]eq "CNV_chr1_2893581_2896880");
#	  	  print OUT "$pre_text\t\t$text\n";
  }
    
}

open VAR, ">>/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/variable_CNVs_per_breed.190425.txt" or die;;
open PRI, ">>/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/CNV/private_CNVs_per_breed.190425.txt" or die;;
foreach $breed (%VARIABLE) {
    print VAR "$breed\t$VARIABLE{$breed}\n";
    
}
close VAR;

foreach $breed (%PRIVATE) {
    print PRI "$breed\t$PRIVATE{$breed}\n";
...
open OUT, ">>/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/".$SV_TYPE."/Segregating_and_private_per_breed_".$SV_TYPE."_190425.txt";
...
open OUT, ">>/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/".$SV_TYPE."/Functional_segregating_per_breed_".$SV_TYPE."_190425.txt";
...
open OUT, ">>/proj/uppstore2017236/b2013119/private/GenomeSTRIP/Analyses/ANNOTATED_SV/".$SV_TYPE."/Functional_segregating_per_individual".$SV_TYPE."_190425.txt";
